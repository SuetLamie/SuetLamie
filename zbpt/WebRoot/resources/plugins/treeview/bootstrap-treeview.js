! function(a, b, c, d) {
	...
	var g = function(b, c) {
		return this.$element = a(b), this.elementId = b.id, this.styleId = this.elementId + "-style", this.init(c), {
			...
			addChildNode: a.proxy(this.addChildNode, this), // 扩展
			findNodeById: a.proxy(this.findNodeById, this) // 扩展
		}
	};
	...
	}, g.prototype.findNodeById = function(nodeId, nodes) {
		var rtNode = null;
		var t = this;
		$(nodes).each(function() {
			if(this.id == nodeId) {
				...
				t.findNodeById(nodeId, this.nodes);
			}
		});
		return rtNode;

	}, g.prototype.addChildNode = function(identifiers, childNode) {
		var parentNode = this.findNodeById(identifiers, this.nodes);
