function TeacherOpusHot(config) {
	var t = this;
	...
	this.tableId = "tb_teacherOpusHot";
	...
	this.tableBind = function() {
		$('#' + t.tableId).bootstrapTable({
			url : 'system/bas/teacherOpusHot/getObjGridStore.action', // 请求后台的URL（*）
			method : 'post', // 请求方式（*）
		...
		if (SysPower.exist(id)) {
			...
			$("#" + id).click(function() {
				top.layer.open({
					...
					area : ['900px','550px'],
					maxmin : false,
					content : 'system/sys/requestPageController/queryMain.action?pagePath=system/bas/teacherOpus/teacherOpusHotAdd.jsp&modeId=101_106_101',
