function TeacherOpusHotAdd(config) {
	var t = this;
	...
	this.tableBind = function() {
		$('#' + t.tableId).bootstrapTable({
			...
			height : $(document).height() - $("#search_form").height()
					- $("#teacherOpusHotAddForm").height() - (40 + 5), // 行高，如果没有设置height属性，表格自动根据记录条数觉得表格高度
	...
	this.submitForm = function() {
		...
		// 确认
		$.post("system/bas/teacherOpusHot/add.action", {
				opusid : arrselections[0]['id']
				}, function(data) {
					if (data.execute) {
