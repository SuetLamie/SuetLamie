function CourseTeacherAdd(config) {
	var t = this;

	// 得到查询的参数
	// this.tree = "";
	this.second = null;
	this.toolbarId = "tb_course_teacher_add_toolbar";
	this.tableId = "tb_course_teacher_add";
	...
	this.tableBind = function() {
		$('#' + t.tableId).bootstrapTable({
			...
			height : $(document).height() - $("#search_form").height()
					- $("#courseteacherAddForm").height() - (40 + 5), // 行高，如果没有设置height属性，表格自动根据记录条数觉得表格高度
	...
	this.submitForm = function() {
		...
		// 确认
		$.post("system/bas/courseteacher/add.action", {
