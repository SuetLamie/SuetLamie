function CourseTeacher(config) {
	var t = this;

	// 得到查询的参数
	// this.tree = "";
	this.second = null;
	this.toolbarId = "tb_course_teacher_toolbar";
	this.tableId = "tb_course_teacher";
	...
	this.tableBind = function() {
		$('#' + t.tableId).bootstrapTable({
			url : 'system/bas/courseteacher/getObjGridStore.action', // 请求后台的URL（*）
			method : 'post', // 请求方式（*）
...
//						title : '排序'
		...
		if (SysPower.exist(id)) {
			...
			$("#" + id).click(function() {
				top.layer.open({
					...
					area : ['900px','550px'],
					maxmin : false,
					content : 'system/sys/requestPageController/queryMain.action?pagePath=system/bas/courseTeacher/courseTeacherAdd.jsp&modeId=101_107_101',
		...
		if (SysPower.exist(id)) {
			...
			$("#" + id).click(function() {
				...
				top.layer.confirm('确认要删除选择的数据吗？', {
							...
							$.post("system/bas/courseteacher/delete.action", {
										ids : ids
									}, function(data) {
										if (data.execute) {
	...
	this.buttonBind = function() {
		// 增加
		t.btnAddBind("101_107_101");
		// 删除
		t.btnDeleteBind("101_107_103");
		// 查询
