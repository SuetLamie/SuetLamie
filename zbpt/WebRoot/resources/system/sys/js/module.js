...
function Module() {
	...
	this.gridStore = new Ext.data.Store({
				url : 'system/sys/ModuleController/getObjGridStore.action',
	...
	this.del = function() {
		...
		if (records.length > 0) {
			...
			for (var i = 0; i < records.length; i++) {
				...
				ids += "'" + records[i].get('m_id') + "'";
			}
			ExtMsg.confirm('确定要删除所选项吗？', function(btn) {
						if (btn == 'yes') {
							var del = PassOwner
									.RequestOfPost(
											'system/sys/ModuleController/delete.action',
											{
												ids : ids
											}, false);
							msgShow(del.msg);
			...
			items : [{
						...
						handler : function() {
							...
							t.form.getForm().findField('isAdd')
									.setValue("true");
							t.form.getForm().findField('m_id').setReadOnly(false);
						}
					}, {
						xtype : 'tbseparator',
						ref : '../addObj_temp'
	...
	this.getSearchParams = function() {
		var params = {
			parentId : t.search2.getValue(),
			m_name : t.search1.getValue()
		};
		return params;
	};
	...
	this.form = new Ext.form.FormPanel({
		...
		items : [new Ext.form.FieldSet({
			...
			items : [{
						...
						items : [{
									xtype : 'combo',
									fieldLabel : '<font color=#467CD5>类型</font>',
									name : 'parentId',
									hiddenName : 'parentId',
									editable : false,// 不让编辑
									forceselection : true,
						...
						items : [{
									xtype : 'numberfield',
									fieldLabel : '<font color=#467CD5>排序</font>',
									name : 'idx',
									allowBlank : false,
									blankText : '排序不能为空!',
									maxValue : 100,
									maxValueText : '排序不能超过100个字符!',
									listeners : {
										scope : this
									}
								}]
			...
			handler : function() {
				if (t.form.getForm().isValid() == true) {
					t.form.getForm().submit({
						url : 'system/sys/ModuleController/saveOrUpdate.action',
						...
						success : function(form, action) {
							...
							if (data.execute) {
								gridLoad(t.grid, t.gridPageSize, 'm_id', t.form
												.getForm().findField('m_id')
												.getValue());
								msgShow(data.msg);
								t.win.hide();
							} else {
