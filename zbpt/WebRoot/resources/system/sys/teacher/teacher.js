function TeacherInfo(config) {
	...
	this.tableId = "tb_teacher";
	this.queryParams = function(params) {
		var temp = { // 这里的键的名字和控制器的变量名必须一直，这边改动，控制器也需要改成一样的
			limit : params.limit, // 页面大小
			start : params.offset, // 页码
			nickname : $("#search_username").val()
		};
		return temp;
	};
	...
	this.tableBind = function() {
		$('#' + t.tableId).bootstrapTable({
			url : 'system/sys/teacher/getObjGridStore.action', // 请求后台的URL（*）
			method : 'post', // 请求方式（*）
			...
			$("#" + id).click(function() {
				top.layer.open({
					...
					area : ['1100px',
							'550px'],
					maxmin : false,
					content : 'system/sys/requestPageController/queryMain.action?pagePath=system/sys/teacher/teacherAdd.jsp&modeId=102_101_101',
